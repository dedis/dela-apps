...
func newDelaNode(path string, port int) (dela, error) {
	...
	if err != nil {
		return nil, xerrors.Errorf("failed to create KV store: %v", err)
	}
	...
	if err != nil {
		return nil, xerrors.Errorf("failed to load or create cert: %v", err)
	}

	key, err := x509.ParseECPrivateKey(keydata)
	if err != nil {
		return nil, xerrors.Errorf("failed to parse ECPrivate key: %v", err)
	}
	...
	if err != nil {
		return nil, xerrors.Errorf("faoéed to create mino grpc: %v", err)
	}
	...
	if err != nil {
		return nil, xerrors.Errorf("failed to load or create signer: %v", err)
	}

	signer, err := bls.NewSignerFromBytes(signerdata)
	if err != nil {
		return nil, xerrors.Errorf("failed to create signer: %v", err)
	}
	...
	if err != nil {
		return nil, xerrors.Errorf("failed to create new pool: %v", err)
	}
	...
	err = tree.Load()
	if err != nil {
		return nil, xerrors.Errorf("failed to load tree: %v", err)
	}
	...
	err = genstore.Load()
	if err != nil {
		return nil, xerrors.Errorf("failed to load genstore: %v", err)
	}
...
// create an new chain.
func (c cosiDelaNode) Setup(delas ...dela) error {
	// share the certificates
	joinable, ok := c.onet.(minogrpc.Joinable)
	if !ok {
		return xerrors.Errorf("not joinable: %T", c.onet)
	}
	...
	if err != nil {
		return xerrors.Errorf("failed to hash certificate: %v", err)
	}

	for _, dela := range delas {
		otherJoinable, ok := dela.GetMino().(minogrpc.Joinable)
		if !ok {
			return xerrors.Errorf("other not joinable: %T", dela.GetMino())
		}
	...
	if !ok {
		return xerrors.Errorf("not extendedService: %T", c.GetOrdering())
	}
	...
	for i, dela := range delas {
		...
		d, ok := dela.(cosiDela)
		if !ok {
			return xerrors.Errorf("not cosiDela: %T", dela)
		}
